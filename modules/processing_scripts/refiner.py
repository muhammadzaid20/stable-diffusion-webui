...
class ScriptRefiner(scripts.ScriptBuiltinUI):
    ...
    def ui(self, is_img2img):
        with InputAccordion(False, label="Refiner", elem_id=self.elem_id("enable")) as enable_refiner:
            with gr.Row():
                refiner_checkpoint = with_webui_tooltip(
                    gr.Dropdown(
                        label='Checkpoint',
                ...
                refiner_switch_at = with_webui_tooltip(
                    gr.Slider(
                        value=0.8,
                        label="Switch at",
                        minimum=0.01,
                    ...
                    "fraction of sampling steps when the switch to refiner model should happen; 1=never, 0.5=switch in the middle of generation",
                )
